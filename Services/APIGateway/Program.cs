using APIGateway.Services;
using APIGateway.Tools;
using Pastebin.Infrastructure.SDK.Services;
using Prometheus;
using Serilog;
using Serilog.Context;

namespace APIGateway
{
    /// <summary>
    /// Ãëàâíûé êëàññ ïðîãðàììû
    /// </summary>
    public class Program
    {
        /// <summary>
        /// Òî÷êà âõîäà â ïðèëîæåíèå
        /// </summary>
        /// <param name="args">Àðãóìåíòû êîìàíäíîé ñòðîêè</param>
        public static void Main(string[] args)
        {
            var builder = InitializationWebApplicationBuilder(args);
            var app = InitializationWebApplication(builder);

            app.Run();
        }

        /// <summary>
        /// Èíèöèàëèçàöèÿ è êîíôèãóðàöèÿ âåá-ïðèëîæåíèÿ
        /// </summary>
        /// <param name="builder">Îáúåêò WebApplicationBuilder</param>
        /// <returns>Ñêîíôèãóðèðîâàííîå âåá-ïðèëîæåíèå</returns>
        private static WebApplication InitializationWebApplication(WebApplicationBuilder builder)
        {
            var app = builder.Build();

            // Âêëþ÷åíèå ëîãèðîâàíèÿ çàïðîñîâ Serilog.
            app.UseSerilogRequestLogging();

            // Êîíôèãóðàöèÿ äëÿ ðåæèìà ðàçðàáîòêè.
            if (app.Environment.IsDevelopment())
            {
                app.UseSwagger();
                app.UseSwaggerUI();
            }

            // Âêëþ÷åíèå àóòåíòèôèêàöèè è àâòîðèçàöèè.
            app.UseAuthentication();
            app.UseAuthorization();
            // Ëîãèðîâàíèå HTTP-çàïðîñîâ.
            app.UseSerilogRequestLogging();

            // Middleware äëÿ äîáàâëåíèÿ TraceId â ëîã.
            app.Use(async (context, next) =>
            {
                var traceId = context.TraceIdentifier;
                using (LogContext.PushProperty("TraceId", traceId))
                {
                    await next();
                }
            });

            // Middleware äëÿ ëîãèðîâàíèÿ çàïðîñîâ è îòâåòîâ.
            //app.Use(async (context, next) =>
            //{
            //    var request = context.Request;
            //    var requestBody = await new StreamReader(request.Body).ReadToEndAsync();
            //    Log.Information("HTTP Request {Method} {Path} {Query} {Body}", request.Method, request.Path, request.QueryString, requestBody);

            //    var originalResponseBodyStream = context.Response.Body;
            //    using var responseBodyStream = new MemoryStream();
            //    context.Response.Body = responseBodyStream;

            //    await next();

            //    context.Response.Body.Seek(0, SeekOrigin.Begin);
            //    var responseBody = await new StreamReader(context.Response.Body).ReadToEndAsync();
            //    context.Response.Body.Seek(0, SeekOrigin.Begin);

            //    Log.Information("HTTP Response {StatusCode} {Body}", context.Response.StatusCode, responseBody);
            //    await responseBodyStream.CopyToAsync(originalResponseBodyStream);
            //});

            app.UseMetricServer();
            // Ñ÷èòàåì çàïðîñû, îøèáêè, âðåìÿ îáðàáîòêè.
            app.UseHttpMetrics();

            // Ìàðøðóòèçàöèÿ êîíòðîëëåðîâ.
            app.MapControllers();

            return app;
        }

        /// <summary>
        /// Èíèöèàëèçàöèÿ è êîíôèãóðàöèÿ WebApplicationBuilder.
        /// </summary>
        /// <param name="args">Àðãóìåíòû êîìàíäíîé ñòðîêè.</param>
        /// <returns>Ñêîíôèãóðèðîâàííûé WebApplicationBuilder.</returns>
        private static WebApplicationBuilder InitializationWebApplicationBuilder(string[] args)
        {
            var builder = WebApplication.CreateBuilder(args);

            // Êîíôèãóðàöèÿ Serilog.
            Log.Logger = new LoggerConfiguration()
                .ReadFrom.Configuration(builder.Configuration)
                .CreateLogger();

            builder.Host.UseSerilog();

            var authServiceUrl = conf.GetConnectionString("authservice");
            if (string.IsNullOrEmpty(authServiceUrl))
                throw new InvalidOperationException("Auth service url is not configured.");
            services.AddHttpClient<UserServices>(c => c.BaseAddress = new Uri(authServiceUrl));
            InitializationServices(builder.Services, builder.Configuration);

            return builder;
        }

        /// <summary>
        /// Èíèöèàëèçàöèÿ ñåðâèñîâ
        /// </summary>
        /// <param name="services">Êîëëåêöèÿ ñåðâèñîâ</param>
        /// <param name="conf">Êîíôèãóðàöèÿ ïðèëîæåíèÿ</param>
        private static void InitializationServices(IServiceCollection services, IConfiguration conf)
        {
            services.AddControllers();
            services.AddEndpointsApiExplorer();
            services.AddSwaggerGen();

            // Êîíôèãóðàöèÿ RabbitMQ
            var rabbitMqConnectionString = conf.GetConnectionString("rabbitmq");
            if (string.IsNullOrEmpty(rabbitMqConnectionString))
                throw new InvalidOperationException("RabbitMQ connection string is not configured.");
            services.AddSingleton(new RabbitMqService(rabbitMqConnectionString));

            // Êîíôèãóðàöèÿ JWT àâòîðèçàöèè
            services.AddJWTAuthorization("Test");
            services.AddTransient<UserServices>();
            services.AddTransient<PostsService>();
        }
    }
}
